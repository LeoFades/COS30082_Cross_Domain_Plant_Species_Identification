# -*- coding: utf-8 -*-
"""
MobileNetV2 PyTorch Training - Cross Domain Plant Species Identification
...
SAVE_DIR = os.path.join(BASE_DIR, 'Approach_1/MobileNet/models_pytorch')
...
# ==========================================
# Load Dataset (matching your MobileNet notebook)
# ==========================================
...
train_filenames, train_labels = load_labels(train_txt)
test_filenames, test_labels = load_labels(groundtruth_txt)
...
# Full paths
train_paths = [os.path.join(EXTRACT_DIR, fname) for fname in train_filenames]
test_paths = [os.path.join(EXTRACT_DIR, fname) for fname in test_filenames]
...
# ==========================================
# Transforms (matching your TF augmentation)
# ==========================================
train_transform = transforms.Compose([
    transforms.Resize((IMG_SIZE, IMG_SIZE)),
    transforms.RandomHorizontalFlip(),
    transforms.RandomRotation(10),  # RandomRotation(0.1)
    transforms.RandomAffine(degrees=0, scale=(0.9, 1.1)),  # RandomZoom(0.1)
    transforms.ToTensor(),
...
test_dataset = PlantDataset(test_paths, test_labels, test_transform)
...
class PlantMobileNetV2(nn.Module):
    ...
    def unfreeze_last_n_layers(self, n=30):
        ...
        all_params = [(name, param) for name, param in self.backbone.named_parameters() if 'classifier' not in name]
...
# Initialize Model
# ==========================================
model = PlantMobileNetV2(NUM_CLASSES).to(DEVICE)
...
for epoch in range(1, EPOCHS_FROZEN + 1):
    ...
    if val_acc > best_val_acc:
        ...
        patience_counter = 0
        torch.save(model.state_dict(), os.path.join(SAVE_DIR, 'mobilenetv2_frozen_best.pth'))
...
model.load_state_dict(torch.load(os.path.join(SAVE_DIR, 'mobilenetv2_frozen_best.pth')))
...
for epoch in range(1, EPOCHS_UNFROZEN + 1):
    ...
    if val_acc > best_val_acc:
        best_val_acc = val_acc
        patience_counter = 0
        torch.save(model.state_dict(), os.path.join(SAVE_DIR, 'mobilenetv2_plant.pth'))
        print(f"Saved best model (acc: {best_val_acc:.2f}%)")
    else:
        patience_counter += 1
        if patience_counter >= patience:
...
plt.tight_layout()
plt.savefig(os.path.join(SAVE_DIR, 'mobilenetv2_training_curves.png'), dpi=300)
plt.show()
...
model.load_state_dict(torch.load(os.path.join(SAVE_DIR, 'mobilenetv2_plant.pth')))
...
plt.tight_layout()
plt.savefig(os.path.join(SAVE_DIR, 'mobilenetv2_cm_best20.png'), dpi=300)
plt.show()
...
plt.ylabel("True Class ID")
plt.tight_layout()
plt.savefig(os.path.join(SAVE_DIR, 'mobilenetv2_cm_worst20.png'), dpi=300)
plt.show()
...
report_df.to_csv(os.path.join(SAVE_DIR, 'mobilenetv2_classification_report.csv'))
...
for i in range(min(20, NUM_CLASSES)):
    ...
    correct = cm[i, i] * total_samples
    incorrect_indices = np.argsort(cm[i])[::-1]
    top_confused = []
    for j in incorrect_indices:
        if j != i and cm[i, j] > 0:
            confused_class = idx_to_label[j]
            group = "WITH pairs" if j in paired_classes else "WITHOUT pairs"
            top_confused.append(f"{confused_class} ({group}): {cm[i,j]*total_samples:.0f}")
        if len(top_confused) >= 3:
            break
    
    group_i = "WITH pairs" if i in paired_classes else "WITHOUT pairs"
    print(f"Class {true_class} ({group_i}): {correct:.0f}/{total_samples} correct -> {', '.join(top_confused)}")

print("\n" + "="*60)
print("Training Complete!")
print(f"Best model saved to: {os.path.join(SAVE_DIR, 'mobilenetv2_plant.pth')}")
print("="*60)
