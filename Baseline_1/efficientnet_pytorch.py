# -*- coding: utf-8 -*-
"""
EfficientNet PyTorch Training - Cross Domain Plant Species Identification
...
SAVE_DIR = os.path.join(BASE_DIR, 'Approach_1/Mic/models_pytorch')
...
train_transform = transforms.Compose([
    ...
    transforms.RandomRotation(20),  # RandomRotation(0.2) in TF = 0.2 * 360 degrees
    transforms.RandomAffine(degrees=0, scale=(0.75, 1.25)),  # RandomZoom(0.25)
...
class PlantEfficientNet(nn.Module):
    def __init__(self, num_classes):
        ...
        # GlobalAveragePooling2D -> BN -> Dense(256, swish, L2) -> Dropout(0.3) -> Dense(100)
        in_features = self.backbone.classifier[1].in_features
        self.backbone.classifier = nn.Sequential(
            nn.Dropout(0.2, inplace=True),  # Original EfficientNet dropout
    ...
    def unfreeze_last_n_layers(self, n=40):
        """Unfreeze last n layers, keeping BN frozen"""
        # First freeze everything except classifier
        for name, param in self.backbone.named_parameters():
            if 'classifier' not in name:
                param.requires_grad = False
        
        # Unfreeze last n layers (but keep BatchNorm frozen as in your TF code)
        ...
        for name, param in all_params[-n:]:
            if 'bn' not in name.lower() and 'batch' not in name.lower():
...
# Initialize Model
# ==========================================
model = PlantEfficientNet(NUM_CLASSES).to(DEVICE)
...
for epoch in range(1, EPOCHS_FROZEN + 1):
    ...
    if val_acc > best_val_acc:
        ...
        patience_counter = 0
        torch.save(model.state_dict(), os.path.join(SAVE_DIR, 'efficientnet_frozen_best.pth'))
...
# ==========================================
print("\n" + "="*60)
print("Stage 2: Fine-tuning with Last 40 Layers Unfrozen")
print("="*60)

model.load_state_dict(torch.load(os.path.join(SAVE_DIR, 'efficientnet_frozen_best.pth')))
model.unfreeze_last_n_layers(40)

optimizer = optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=LR_UNFROZEN, weight_decay=1e-7)
...
for epoch in range(1, EPOCHS_UNFROZEN + 1):
    ...
    if val_acc > best_val_acc:
        best_val_acc = val_acc
        patience_counter = 0
        torch.save(model.state_dict(), os.path.join(SAVE_DIR, 'efficientnet_plant.pth'))
        print(f"Saved best model (acc: {best_val_acc:.2f}%)")
    else:
        patience_counter += 1
        if patience_counter >= patience:
...
plt.axvline(x=frozen_epochs-1, color='red', linestyle='--', label='Unfreeze last 40 layers')
plt.xlabel('Epoch')
plt.ylabel('Accuracy (%)')
...
plt.tight_layout()
plt.savefig(os.path.join(SAVE_DIR, 'efficientnet_training_curves.png'), dpi=300)
plt.show()
...
model.load_state_dict(torch.load(os.path.join(SAVE_DIR, 'efficientnet_plant.pth')))
...
plt.tight_layout()
plt.savefig(os.path.join(SAVE_DIR, 'efficientnet_cm_best20.png'), dpi=300)
plt.show()
...
plt.ylabel("True Class ID")
plt.tight_layout()
plt.savefig(os.path.join(SAVE_DIR, 'efficientnet_cm_worst20.png'), dpi=300)
plt.show()
...
report_df.to_csv(os.path.join(SAVE_DIR, 'efficientnet_classification_report.csv'))
print(f"\nSaved classification report to {SAVE_DIR}")

print("\n" + "="*60)
print("Training Complete!")
print(f"Best model saved to: {os.path.join(SAVE_DIR, 'efficientnet_plant.pth')}")
print("="*60)
